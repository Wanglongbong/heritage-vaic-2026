  ...
  return <section className="hand-viewer" aria-label={`${ui.title}: ${label}`}>
    ...
    <div className="hand-viewer-stage" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={() => { draggingRef.current = false; }} onPointerCancel={() => { draggingRef.current = false; }}>
      <div ref={objectRef} className="tracked-object" style={{ "--object-rx": "-8deg", "--object-ry": "0deg", "--object-zoom": 1 } as CSSProperties}>
        {viewSources.map((source, index) => <Image key={source} className={`tracked-object-face ${activeView === index ? "is-active" : ""}`} src={source} alt={activeView === index ? `${label} · ${ui.views[index]}` : ""} aria-hidden={activeView !== index} fill unoptimized sizes="(max-width: 640px) 72vw, 340px" draggable={false} style={{ objectFit: "contain" }} />)}
        <b>{label}</b>
